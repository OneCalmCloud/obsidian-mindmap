import { type App as ObsidianApp, Plugin, PluginSettingTab, TextFileView, moment, WorkspaceLeaf, Setting, Menu, TFile, normalizePath, TFolder, MarkdownView, ViewState, requireApiVersion } from "obsidian";
...
export class MindmapView extends TextFileView {
  ...
  onload(): void {
    // Ensure the view-content area has no padding for full-bleed canvas.
    this.contentEl.classList.add("mindmap-view-content");
    ...
    let MyComponent = defineComponent({
      setup() {
        ...
        const zoomChange = (value: string) => {
          //@ts-ignore
          const leafId = self.leaf.id as string;
          let find = appStore.viewTransforms.find((viewTransform) => viewTransform.leafId === leafId);
          if (find) {
        ...
        //@ts-ignore
        const leafId = self.leaf.id as string;
        return () => <MindmapAppView local={getLocal()} activeLeafId={leafId} render={shouldRender.value} settings={self.plugin.settings} fileIds={self.fileIds.value} freshKey={self.key.value} data={b.value} onZoomChange={zoomChange} onUpdate={saveFile} />;
      },
    });
    ...
    // IMPORTANT: mount into `.view-content` so we don't overwrite `.view-header`.
    this.contentEl.empty();
    this.appEl.mount(this.contentEl);
  }
  getViewData() {
    return this.rawData;
  }
...
export default class MindmapPlugin extends Plugin {
  ...
  async onload() {
    this.ensureMindmapFadeoutStyles();
    ...
    try {
      ...
      console.error("[mindmap] Error setting up editor handler", e);
    }

    // Try to switch earlier than active-leaf-change.
  ...
  private ensureMindmapFadeoutStyles() {
    const doc = this.app?.workspace?.containerEl?.ownerDocument ?? document;
    const id = "mindmap-cm6-fadeout-style";
    if (doc.getElementById(id)) return;
    const style = doc.createElement("style");
    style.id = id;
    style.textContent = `
      .mindmap-opacity-0 { opacity: 0 !important; }
      .workspace-leaf.mindmap-hide-inline-title .inline-title { display: none !important; }
      .mindmap-view-content { padding: 0 !important; }
    `.trim();
