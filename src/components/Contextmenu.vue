...
import type { MenuItem } from './Mindmap/variable/contextmenu'
import { getCurrentLeafId } from './Mindmap/instance'
import i18n from '../i18n'
import { PropType, Ref, defineComponent, nextTick, reactive, ref, useCssModule } from 'vue'

export default defineComponent({
  name: 'contextmenu',
  props: {
    /** 仅响应对应分屏的右键菜单广播，避免多开 mindmap 时全局 mitt 误触发所有实例 */
    mindmapLeafId: { type: String, default: "" },
    position: {
  ...
  setup (props, context) {
    ...
    emitter.on('showContextmenu', async (val: boolean | { leafId: string; show: boolean }) => {
      let payload: { leafId: string; show: boolean };
      if (typeof val === "boolean") {
        if (!val) {
          show.value = false;
          return;
        }
        payload = { leafId: getCurrentLeafId(), show: true };
      } else {
        payload = val;
      }
      if (!payload.show) {
        if (!payload.leafId || payload.leafId === props.mindmapLeafId) {
          show.value = false;
        }
        return;
      }
      if (payload.leafId !== props.mindmapLeafId) {
        show.value = false;
        return;
      }
