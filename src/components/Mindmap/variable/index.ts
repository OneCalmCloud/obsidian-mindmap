...
import { Ref, ref, type SetupContext } from "vue";
...
const zoomTransformByLeaf = new Map<string, Ref<d3.ZoomTransform>>();
export const getZoomTransformRef = (leafId = getCurrentLeafId()): Ref<d3.ZoomTransform> => {
  const id = String(leafId || "");
  let r = zoomTransformByLeaf.get(id);
  if (!r) {
    r = ref(d3.zoomIdentity) as Ref<d3.ZoomTransform>;
    zoomTransformByLeaf.set(id, r);
  }
  return r;
};
export const zoomTransform: Ref<d3.ZoomTransform> = getZoomTransformRef();
