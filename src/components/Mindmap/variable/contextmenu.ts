...
export type MenuEvent = "zoomin" | "zoomout" | "zoomfit" | "add" | "delete" | "selectall" | "collapse" | "expand" | "add-sibling" | "add-sibling-before" | "add-parent" | "copy" | "paste" | "cut" | "delete-one" | "add-image" | "clean";
...
export const addImage: Ref<MenuItem> = ref({ name: "add-image", disabled: false });
export const cleanItem: Ref<MenuItem> = ref({ name: "clean", disabled: false });

const nodeMenu = computed<MenuItem[][]>(() =>
  [
    [addItem.value, addParentItem.value, addSiblingItem.value, addSiblingBeforeItem.value],
    [cutItem.value, copyItem.value, pasteItem.value, deleteItem.value, deleteOneItem.value, cleanItem.value],
