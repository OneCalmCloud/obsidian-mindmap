...
import { ctm, editFlag, selection, textRectPadding, getZoomTransformRef } from "../variable";
...
import { add, addParent, addSibling, changeLeft, collapse, del, delOne, expand, getMmdata, moveChild, moveSibling, rename } from "../data";
...
export const onZoomMove = (e: d3.D3ZoomEvent<SVGSVGElement, null>): void => {
  ...
  getZoomTransformRef().value = e.transform;
...
export const onEditBlur = (): void => {
  const dragBoxStore = useDragBoxStore();
  const { g } = selection;
  g?.select<SVGGElement, unknown>(`g.${style.edited}`).each(function () {
    this.classList.remove(style.edited, style.selected);
  });
  const { foreignEle, foreignDivEle } = getElements();
...
export const onContextmenu = (e: MouseEvent): void => {
  ...
  const leafId = host?.getAttribute("data-mindmap-leaf") ?? "";
  if (leafId) {
  ...
  emitter.emit("showContextmenu", { leafId, show: true });
};
...
export const onClickMenu = (name: MenuEvent): void => {
  switch (name) {
    ...
    case "cut":
      {
        ...
        const rawdata = getMmdata().find(id)?.rawData;
        if (rawdata) {
    ...
    case "copy":
      {
        const seleData = getSelectedGData();
        const rawdata = getMmdata().find(seleData.id)?.rawData;
        if (rawdata) {
          // navigator.clipboard.write
          navigator.clipboard.writeText(JSON.stringify(rawdata));
        }
    ...
    case "add-image":
      {
        const appStore = useAppStore();
        const seleData = getSelectedGData();
        const { uploadImageFileInput } = getElements();
          ...
          uploadImageFileInput.value.onchange = (event) => {
            ...
            reader.onload = async () => {
              ...
              if (appStore.app && binaryData) {
                // In some environments `File` won't have a `path` field.
                // When that happens, we always import it into the vault.
                const basePath = (appStore.app.vault.adapter as any)?.basePath as string | undefined;
                const pickedPath = (file as any)?.path as string | undefined;
                const canResolveInVault = !!(basePath && pickedPath);

                if (canResolveInVault) {
                  const reg = new RegExp("^" + basePath.replace(/\\/g, "\\\\"));
                  const filePath = pickedPath!.replace(reg, "");
                  if (!(await appStore.app.vault.adapter.exists(filePath))) {
                    // Not inside vault, fall through to importing.
                  } else {
                    const linkPath = filePath.replace(/^\\/, "").replace(/\\/g, "/");
...
export function onDragMove(this: SVGGElement, e: d3.D3DragEvent<SVGGElement, Mdata, Mdata>, d: Mdata): void {
  ...
  g.selectAll<SVGGElement, unknown>("g.node").each(function () {
    ...
    if (el.classList.contains(style.outline) && el !== n) {
      el.classList.remove(style.outline);
    }
  });
...
export function onDragEnd(this: SVGGElement, e: d3.D3DragEvent<SVGGElement, Mdata, Mdata>, d: Mdata): void {
  ...
  // 判断是否找到了新的父节点
  const { g } = selection;
  let np: SVGGElement | null = null;
  g?.selectAll<SVGGElement, unknown>("g.node").each(function () {
    if (this.classList.contains(style.outline)) {
      np = this;
    }
  });
  if (np) {
  ...
  // 判断是否变换left
  const xToCenter = d.x - mm.getRootWidth() / 2;
