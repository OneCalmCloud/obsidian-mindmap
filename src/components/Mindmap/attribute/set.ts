 ...
 * 根据该节点是否是根节点，绘制不同的效果
 */
export const attrA = (isRoot: boolean, gTrigger: SelectionRect, gTextRect: SelectionRect, gExpandBtn: SelectionG, gAddBtn?: SelectionG): void => {
  if (isRoot) {
...
export const attrImg = (tspan: d3.Selection<SVGImageElement, unknown, null, undefined>, data: string): void => {
  const appStore = useAppStore();

  let imgData = JSON.parse(data);
  if (imgData.fileId) {
    let find = appStore.fileIds.find((fileId) => fileId.id === imgData.fileId);
    if (find) {
      let linkpathDest = appStore.app.metadataCache.getFirstLinkpathDest(`${find.value}`, "/");
      if (linkpathDest) {
        imgData.src = appStore.app.vault.getResourcePath(linkpathDest);
      }
    }
  }
  tspan
    .attr("height", 38 * 2)
    .attr("xlink:href", imgData.src)
    .attr("href", imgData.src)
    .attr("width", imgData.width)
    .attr("height", imgData.height)
    .attr("transform", (d) => `translate(${d.left ? -d.width : 0},0)`);
};
...
export const attrPath = (p: d3.Selection<SVGPathElement, Mdata, SVGGElement, IsMdata>, tran?: Transition): void => {
